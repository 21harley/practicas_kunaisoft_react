...
import {addCant,sumar} from '../../reducers/Redux_toolkit/slices/carrito';
...
const ShoItem = ({
  ...
  const {carritoComp} =useSelector(state=>state.carrito);
    ...
    <li key={el.id} className="h-[380px] w-[380px] bg-slate-100 grid mt-5">
        ...
        <div className="flex">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (enCarrito) {
                dispatch(addCant({ type: "addCant",  p:pos, cant:Number(e.target.value)}));
      ...
      <div className="grid h-[80px]">
        <button
          className={
            !enCarrito
              ? "bg-black hover:bg-green-600 text-white h-full"
              : "bg-black hover:bg-red-800 text-white"
          }
          onClick={() => {
            entradaCarrito(pos, totalItem);
          }}
        >
